...
export const ClubDetailsPage = () => {
  ...
  if (clubQuery.isError || !clubQuery.data)
    return <main className={styles.state}>{t("clubs.notFound")}</main>;
  ...
  return (
    <main className={styles.page}>
      <Link className={styles.back} to={routes.clubs}>
        ← {t("clubs.backToClubs")}
      </Link>
      ...
      <nav className={styles.tabs} aria-label={t("clubs.clubNavigation")}>
        <NavLink end to={`/clubs/${id}`}>
          {t("clubs.overview")}
        </NavLink>
        <NavLink to={`/clubs/${id}/games`}>{t("clubs.games")}</NavLink>
        <NavLink to={`/clubs/${id}/tournaments`}>
          {t("clubs.tournaments")}
        </NavLink>
        <NavLink to={`/clubs/${id}/leaderboards`}>
          {t("clubs.leaderboards")}
        </NavLink>
      </nav>
        ...
        <div className={styles.mainContent}>
          ...
          <section className={styles.contentSection}>
            <div className={styles.sectionHeading}>
              <div>
                <p>{t("clubs.libraryLabel")}</p>
                <h2>{t("clubs.availableGames")}</h2>
              </div>
          ...
          <section className={styles.contentSection}>
            <div className={styles.sectionHeading}>
              <div>
                <p>{t("clubs.eventsLabel")}</p>
                <h2>{t("clubs.upcomingTournaments")}</h2>
              </div>
              <Link to={`/clubs/${id}/tournaments`}>
                {t("clubs.viewAllTournaments")} →
              </Link>
            </div>
