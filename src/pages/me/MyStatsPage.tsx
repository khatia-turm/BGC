...
export const MyStatsPage = () => {
  ...
  const gameNames = new Map(
    (games.data ?? []).map((game) => [game.id, game.title]),
  );
  return (
      ...
      {profile && (
        <section className={styles.stats}>
          <div className={styles.stat}>
            <strong>{profile.stats.ratingPoints.toFixed(1)}</strong>
        ...
        <section className={styles.grid}>
          {profile.rankings.map((ranking) => (
            <article
              ...
              key={`${ranking.gameId}-${ranking.season}`}
            >
              <p>{gameNames.get(ranking.gameId) ?? "Board game"}</p>
              <h2>#{ranking.rank}</h2>
              <div className={styles.meta}>
                <span>{ranking.season}</span>
                <span>{ranking.ratingPoints.toFixed(1)} points</span>
              </div>
            </article>
          ))}
        </section>
