...
export const MyHistoryPage = () => {
  ...
  const history = (player.data?.tournaments ?? []).filter(
    (event) => new Date(event.startsAt).getTime() < openedAt,
  );
  return (
    <main className={styles.page}>
      ...
      {history.length ? (
        <section className={styles.list}>
          {history.map((event) => (
            <article className={styles.card} key={event.tournamentId}>
              <span className={styles.status}>{event.status}</span>
              <h2>{event.name}</h2>
              <p>
                {new Date(event.startsAt).toLocaleDateString()} · {event.city}
              </p>
            </article>
          ))}
        </section>
