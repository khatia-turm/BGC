...
export const DashboardPage = () => {
  ...
  const registeredPlayers = own.reduce((total,item) => total + item.registeredPlayers,0);
  return <main className={`${styles.page} ${styles.dashboard}`}>
    ...
    <section className={styles.stats}>
      <article className={styles.stat}><span>Active Tournaments</span><strong>{active.length}</strong><small>{active.filter((t)=>t.status==="InProgress").length} in progress, {upcoming.length} upcoming</small></article>
      <article className={styles.stat}><span>Registered Players</span><strong>{registeredPlayers}</strong><small>Seats reserved across tournaments</small></article>
      <article className={styles.stat}><span>Game Inventory</span><strong className={styles.altNumber}>{games.data?.length ?? 0}</strong><small>Board games listed by this club</small></article>
