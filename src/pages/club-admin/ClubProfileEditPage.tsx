...
export const ClubProfileEditPage = () => {
  ...
  if (!club.data)
    return <main className={styles.page}>Loading club settings…</main>;
  return (
    <Editor
      key={club.data.updatedAt}
      clubId={clubId}
      initial={{
        name: club.data.name,
...
const Editor = ({
  ...
  clubId: number;
  initial: CreateClubPayload;
}) => {
  const [form, setForm] = useState(initial);
  const [saved, setSaved] = useState(false);
  const [imageError, setImageError] = useState<string | null>(null);
  const update = useUpdateClub(clubId);
  const field =
    (key: keyof CreateClubPayload) =>
  ...
  const chooseLogo = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImageError(null);
    try {
      setForm({ ...form, logoUrl: await readImageFileAsDataUrl(file) });
    } catch (error) {
      setImageError(
  ...
  return (
    <main className={styles.page}>
      <header>
        <div>
          <h1>Settings</h1>
          <p>Manage your club’s public identity and contact information</p>
        </div>
        <Link to={`/clubs/${clubId}`}>View public profile ↗</Link>
      </header>
      <form onSubmit={submit}>
        <section className={styles.section}>
          <div className={styles.sectionHeading}>
            ...
            <div>
              <h2>Club identity</h2>
              <p>This is how players recognize your club across MeepleHub.</p>
            </div>
          </div>
          <div className={styles.identity}>
            ...
            <div className={styles.identityFields}>
              ...
              <label>
                ...
                <small>
                  Square images work best. Files must be under 450 KB.
                </small>
              </label>
            </div>
          </div>
        ...
        <section className={styles.section}>
          <div className={styles.sectionHeading}>
            ...
            <div>
              ...
              <p>Help players find your tables and know when you’re open.</p>
            </div>
          </div>
          <div className={styles.twoColumns}>
            <label>
              City
              <input value={form.city} onChange={field("city")} required />
            </label>
            <label>
              Working hours
              <input
        ...
        <section className={styles.section}>
          <div className={styles.sectionHeading}>
            ...
            <div>
              ...
              <p>Public contact information for tournament participants.</p>
            </div>
          </div>
          <div className={styles.twoColumns}>
            <label>
        ...
        <footer className={styles.saveBar}>
          <div>
            {saved ? (
              ...
              <span>Unsaved changes are not visible publicly.</span>
            )}
            {update.error && <em>{update.error.message}</em>}
          </div>
