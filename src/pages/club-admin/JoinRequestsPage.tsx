...
export const JoinRequestsPage = () => {
  ...
  return (
    <main className={styles.page}>
      <header>
        <h1>Join Requests</h1>
        <p>Review players who want to join your tournaments</p>
      </header>
      <section className={styles.list}>
        {requests.data?.map((request) => (
          <article className={styles.request} key={request.id}>
            <Link className={styles.player} to={`/players/${request.userId}`}>
              {request.avatarUrl ? (
                <img src={request.avatarUrl} alt="" />
              ) : (
                <span>{request.nickname.charAt(0).toUpperCase()}</span>
              )}
              <div>
                ...
                <small>
                  Requested {new Date(request.registeredAt).toLocaleString()}
                </small>
              </div>
            </Link>
            <div className={styles.tournament}>
              <small>Requested tournament</small>
              <Link to={`../tournaments/${request.tournamentId}`}>
            ...
            <div className={styles.actions}>
              <button
                ...
                onClick={() =>
                  review.mutate({ id: request.id, status: "Accepted" })
                }
              >
                Approve
              </button>
      ...
      {!requests.isLoading && !requests.data?.length && (
        <div className={styles.empty}>
          ...
          <span>New tournament registration requests will appear here.</span>
        </div>
      )}
