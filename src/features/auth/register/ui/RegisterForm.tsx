  ...
  return (
    <main className={`${styles.page} ${styles.registerPage}`}>
      <section className={styles.storyPanel}>
        <div>
          <p className={styles.eyebrow}>{t("auth.registerStoryEyebrow")}</p>
          <h1>{t("auth.registerStoryTitle")}</h1>
          <p className={styles.storyCopy}>{t("auth.registerStoryCopy")}</p>
        </div>
        <ol className={styles.benefits}>
          <li>
            <span>01</span>
            {t("auth.benefitEvents")}
          </li>
          <li>
            <span>02</span>
            {t("auth.benefitStats")}
          ...
          <li>
            <span>03</span>
            {t("auth.benefitCommunity")}
          </li>
        </ol>
        <p className={styles.memberCount}>
          <strong>2,400+</strong> {t("auth.playersJoined")}
        </p>
      </section>
      <section className={styles.formPanel}>
        <div className={`${styles.formWrap} ${styles.registerWrap}`}>
          <p className={styles.step}>{t("auth.joinCommunity")}</p>
          <h2>{t("auth.registerTitle")}</h2>
          <p className={styles.intro}>{t("auth.registerCopy")}</p>
          <form onSubmit={form.submit} className={styles.form}>
            <div className={styles.avatarRow}>
              ...
              <div>
                ...
                <p>{t("auth.photoHint")}</p>
                <label className={styles.upload}>
                  {t("auth.choosePhoto")}
                  <input
                    type="file"
                    accept="image/png,image/jpeg,image/webp"
                    onChange={form.chooseAvatar}
                  />
                </label>
              </div>
            </div>
            ...
            <div className={styles.twoColumns}>
              <label>
                ...
                <input
                  ...
                  onChange={form.update("firstName")}
                  autoComplete="given-name"
                  required
                />
                <FieldError error={form.getFieldError("firstName")} />
              </label>
              <label>
                <span>{t("auth.lastName")}</span>
                <input
                  value={values.lastName}
                  onChange={form.update("lastName")}
                  autoComplete="family-name"
                  required
                />
                <FieldError error={form.getFieldError("lastName")} />
              </label>
            </div>
            ...
            <label>
              ...
              <div className={styles.inputWrap}>
                ...
                <input
                  ...
                  onChange={form.update("nickname")}
                  placeholder="TabletopKing"
                  autoComplete="nickname"
                  required
                />
              </div>
              <FieldError error={form.getFieldError("nickname")} />
            </label>
            ...
            <div className={styles.twoColumns}>
              ...
              <label>
                ...
                <input
                  ...
                  required
                />
                <FieldError error={form.getFieldError("phone")} />
              </label>
            </div>
            ...
            <div className={styles.twoColumns}>
              <label>
                ...
                >
                  <i className={form.passwordScore > 0 ? styles.filled : ""} />
                  <i className={form.passwordScore > 1 ? styles.filled : ""} />
                  <i className={form.passwordScore > 2 ? styles.filled : ""} />
                  <i className={form.passwordScore > 3 ? styles.filled : ""} />
                </div>
                <FieldError
                  error={form.getFieldError("password") ?? form.passwordErrors}
                />
              </label>
              <label>
                ...
                <input
                  ...
                  onChange={form.update("confirmPassword")}
                  autoComplete="new-password"
                  required
                />
                <FieldError
                  error={form.passwordsMismatch ? t("auth.passwordMismatch") : null}
                />
              </label>
            </div>
            ...
            <fieldset className={styles.gameChoices}>
              ...
              {games.map((game) => (
                <label key={game.id}>
                  <input
                    type="checkbox"
                    checked={form.favoriteGameIds.includes(game.id)}
                    onChange={() => form.toggleFavoriteGame(game.id)}
                  />
            ...
            >
              {form.isPending ? t("auth.working") : t("auth.registerAction")}{" "}
              <span>-&gt;</span>
            </button>
          </form>
