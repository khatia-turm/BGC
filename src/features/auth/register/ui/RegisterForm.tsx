    ...
    <section className={styles.formPanel}>
      <div className={`${styles.formWrap} ${styles.registerWrap}`}>
        ...
        <form onSubmit={form.submit} className={styles.form}>
          ...
          <div className={styles.twoColumns}><label><span>Birthday</span><input type="date" value={values.birthday} onChange={form.update("birthday")} max={new Date().toISOString().slice(0, 10)} required /></label><label><span>Gender</span><select value={values.gender} onChange={form.update("gender")} required><option value="" disabled>Select gender</option><option value="0">Male</option><option value="1">Female</option><option value="2">Other</option></select></label></div>
          ...
          <button className={styles.submit} type="submit" disabled={form.isPending || form.passwordsMismatch}>{form.isPending ? t("auth.working") : t("auth.registerAction")} <span>→</span></button>
        </form>
